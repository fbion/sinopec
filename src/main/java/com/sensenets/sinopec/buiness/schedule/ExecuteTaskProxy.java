...
/**
  * @ClassName: ExecuteCollectTaskProxy
  ...
  */
@Slf4j
public class ExecuteTaskProxy extends Thread {
     ...
     * @Fields INSTANCE
     */
    private static volatile ExecuteTaskProxy INSTANCE;
     ...
     */
    public static ExecuteTaskProxy init() {
        if (INSTANCE == null) {
            synchronized (ExecuteTaskProxy.class) {
                if (INSTANCE == null) {
                    ...
                    INSTANCE = new ExecuteTaskProxy();
                    log.info("------ExecuteCollectTaskProxy init ------");
                }
            }
        }
        return INSTANCE;
    ...
    public void run() {
        while (true) {
            if (checkRunning()) {
                try {
                    CollectTaskDto task = ExecuteTaskQueue.take();
                    if (!isExist(task.getResultId())) {
                        if (task.getTaskStatus() == CollectTaskStatusEnum.RUNNING.getCode()) {
                            log.debug(String.format("***************存在采集统计任务，开始统计数据*********"));
                            executeCollect(task);
                        }
                    }
                } catch (Exception e) {
    ...
    private void executeCollect(CollectTaskDto task) {
        TASKEXECUTOR.submit(new ExecuteSubTask(task));
    }

    private boolean checkRunning() {
        if (running) {
            log.debug("采集统计线程正在运行");
        } else {
            log.debug("采集统计线程已停止运行");
        }
        return running;
    }
    ...
    public static void removeKey(Long taskId, boolean isOk) {
        ...
        if (MapUtils.isNotEmpty(taskIdMap) && taskIdMap.containsKey(taskId)) {
            ...
            log.debug("移除正在采集统计的id，采集状态:" + isOk);
        }
