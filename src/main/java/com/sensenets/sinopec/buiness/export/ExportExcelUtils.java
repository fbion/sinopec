...
public class ExportExcelUtils {
     ...
     */
    public void exportExcel(HSSFWorkbook workbook, int sheetNum,
            String sheetTitle, String[] headers, List<List<String>> result,Map<String,ExcelImageDto> downloadImageMap) throws Exception {
        // 生成一个表格
        HSSFSheet sheet = workbook.createSheet();
        workbook.setSheetName(sheetNum, sheetTitle);
        // 设置表格默认列宽度为20个字节
            ...
            for (List<String> m : result) {
                ...
                for (String str : m) {
                    ...
                    if(str.indexOf("#image@")!=-1){
                        cell.setCellStyle(createHyperlinkCellStyle(cell,false));
                        String imageFilePath = downloadImageMap.get(str).getImageBasePath()+"/"+String.format("%s-%d-%d.jpg",downloadImageMap.get(str).getImageFileName(),row.getRowNum(),cell.getColumnIndex()) ;
     ...
     * @return
     */
   private static CellStyle createHyperlinkCellStyle(final Cell cell,boolean isBlod){
